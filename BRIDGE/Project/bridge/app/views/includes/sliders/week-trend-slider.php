<div id="weekTrendCarousel" class="carousel slide section-container" data-interval="3000" data-ride="carousel">
    <div id="weekTrendCarousel" class="carousel slide section-container" data-interval="3000" data-ride="carousel">
        <div class="carousel-inner">
            <?php $first = 1; ?>
            <?php foreach ($data['top_friends_posts'] as $post) : ?>
                <div class="carousel-item<?php if ($first == 1) {
                                                echo ' active';
                                                $first = 0;
                                            }; ?>">
                    <div class="blur-img" style="background-image: url('<?php getFileURL($post['media']); ?>'); background-position: center; background-size: cover; filter: blur(15px)"></div>
                    <div class="activity">
                        <img src="<?php getFileURL($post['media']); ?>" alt="Image">
                        <!-- Activity details -->
                        <div class="activity-author">
                            <a class="fs-c-flex" href="<?php concat(ROOT_URL, 'u/' . $post['username']) ?>">
                                <div><img class="author-img mr-r-5" src="<?php getFileURL($post['main_picture']); ?>" alt="Image"></div>
                                <small class="author-name">@<?php echo $post['username']; ?></small>
                            </a>
                            <div class="light-bubble"></div>
                            <div>
                                <svg role="sm-icon" class="mr-r-5" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <?php $linear_id = $post['id'] . getRandomString(5) . '_linear'; ?>
                                    <circle cx="11" cy="11" r="11" fill="url(#<?php echo $linear_id ?>)" />
                                    ...
                                    <defs>
                                        <linearGradient id="<?php echo $linear_id ?>" x1="15.5" y1="20.5" x2="8.5" y2="3.5" gradientUnits="userSpaceOnUse">
                            ...
                            <small><?php echo getNumberAbbr($post['total_reactions']); ?></small>
                            <div class="light-bubble"></div>
                            <div>
                                <svg role="sm-icon" class="mr-r-5" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <?php $linear_id = $post['id'] . getRandomString(5) . '_linear'; ?>
                            ...
                            <small><?php echo getNumberAbbr($post['total_comments']); ?></small>
                            <div class="light-bubble"></div>
                            <small><a href="<?php concat(ROOT_URL, 'activities/' . $post['post_id']) ?>">Voir</a></small>
                        </div>
                    </div>
                </div>
            <?php endforeach; ?>
        </div>
        <a class="carousel-control-prev light" href="#weekTrendCarousel" role="button" data-slide="prev">
            <span><i class="fas fa-chevron-left fa-fw"></i></span>
            <span class="sr-only">Précédent</span>
        </a>
        <a class="carousel-control-next light" href="#weekTrendCarousel" role="button" data-slide="next">
